...
def udine_solver1(problem:CTT,timesol=600):
    ...
    # 4.Neighbor lectures(lectures of two adjacent courses) should not be assigned in the same period
    for (cid,aid) in problem.lectures:
    ...
    for curricula_id in list(problem.curricula.keys()):
        for d in range(problem.days):
            for pid in range(d*problem.ppd,d*problem.ppd+problem.ppd):
                ...
                model.Add(
                    y1decision+y2decision+y3decision>=3
                )
                model.Add(
...
def scenario1():
    for ds_name in CTT._udine_datasets:
        problem=CTT(ds_name)
        esol=udine_solver1(problem)
        print(esol)
