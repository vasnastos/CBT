...
class Graph
{
    ...
    public:
        ...
        int get_degree(const std::string &v);
        std::vector <std::string> get_neighbors(const std::string &v);
...
{
    private:
        std::map <std::string,int> speriods;
        std::map <int,std::pair <int,int>> speriods_format;
        std::map <std::string,int> srooms;
        Problem *p;
        int cost;
    public:
        
        Solution(const std::string &ds_name);

        ~Solution();

        int compute_cost();
        int compute_normalize_cost();
        bool feasibility();

        void saturation_largest_first();
};
